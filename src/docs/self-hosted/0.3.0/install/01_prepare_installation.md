...
## Kubernetes
Gitpod runs on Kubernetes where Gitpod workspaces are Kubernetes pods. Gitpod is installed on Kubernetes using [Helm](https://helm.sh/).
...
Installing Gitpod on a subdomain works as well. For example:
    ...
    my.gitpod.your-domain.com
    my.gitpod.ws.your-domain.com
...
Usually your Git hosting solution (e.g. GitHub or GitLab) acts as OAuth auth provider. This way we control access to Gitpod while making sure every user has proper access to their Git repository.
...
The caveat of using HTTP is that the builtin Docker registry will not work as Docker/containerd does not pull from insecure registries by default.
...
There is a [plethora of tutorials](https://www.google.com/search?q=letsencrypt+wildcard) about [generating wildcard certificates](https://medium.com/@saurabh6790/generate-wildcard-ssl-certificate-using-lets-encrypt-certbot-273e432794d7) using Let's Encrypt.
...
Follow this [link](../34_https_certs/) for more details on how the certificates are used during installation.
...
The database uses a Kubernetes PersistentVolume. We do not recommend using this option for production.

* External MySQL database: Gitpod requires MySQL in version 5.7 or newer.

## Docker Registry (Optional)
Gitpod builds Docker images during workspace startup. This is necessary for Gitpod to work properly but gives also the opportunity of customizations.
To this end Gitpod requires a container registry where images builds can be pushed.

By default Gitpod ships with a built-in Docker registry but an external registry is recommended. You have the following options:
...
* External Docker registry: Gitpod can connect to your own docker registry. Compared to its built-in counterpart this enables performance gains and access to otherwise private images.

## Bucket Storage (Optional)
Gitpod uses bucket storage to persist the contents of workspaces. Each workspace compressed as a single tar archive which is then uploaded to the bucket.
