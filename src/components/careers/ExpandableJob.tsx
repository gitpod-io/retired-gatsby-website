  ...
  return (
    <StyledExpandableJob>
      <button onClick={toggleIsRendered}>
        <Arrow styles={{ transform: isRendered ? 'rotate(-90deg)' : 'rotate(90deg)' }} />
      </button>
      <h3>{title}</h3>
      <p>{intro}</p>
      {isRendered ? (
        <>
          ...
          <a href={`mailto:career@gitpod.io?subject=Application as ${title}`} target="_blank" className="btn btn--cta">
            Apply Now
          </a>
        </>
      ) : null}
