...
const DocTemplate: React.SFC<DocTemplateProps> = ({ data }) => {
  ...
  const editUrl = data.markdownRemark.fileAbsolutePath.replace(
    /^.*\/src\/docs\//,
    'https://gitpod.io/#https://github.com/gitpod-io/website/blob/master/src/docs/'
  )
  return (
    <>
      <DocsLayout
        canonical={data.markdownRemark.frontmatter.url || `${data.markdownRemark.fields.slug.toLowerCase()}`}
        title={`${menuCtx.thisEntry != undefined ? menuCtx.thisEntry!.title || 'Docs' : 'Docs'}`}
        body={
